...
export default class AppRoot extends React.Component {
	constructor(props) {
		...
		const initState = convertTcombDefToState(props.formDef);
		...
		this.setModeState = this.changeMode.bind(this, { mode: 'state' });
	}
	...
	componentWillReceiveProps(nextProps) {
		const newState = convertTcombDefToState(nextProps.formDef);
		this.setState({
			defs: newState.defs,
	...
	render() {
		...
		const options = () => {
			...
			const isSelectedAvailable = this.state.selected && (this.state.selected === 'root' || flatOrder.indexOf(this.state.selected) > -1);
		...
		if (this.state.mode === 'state') { mode = stateMode; }
